...
// Drag-to-reorder for top-level categories on the /categories settings page.
// Subcategories are rendered inside their parent's section element, so they
// move along with the parent automatically — they are never dragged on their own.
export default class extends Controller {
  ...
  async saveOrder() {
    const orderedIds = this.sectionTargets.map(
    ...
    if (!csrfToken) {
      console.error(
        "[Categories Sortable] CSRF token not found. Cannot save category order.",
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          "[Categories Sortable] Failed to save category order:",
          response.status,
          errorData,
        );
      }
      ...
      console.error(
        "[Categories Sortable] Network error saving category order:",
        error,
      );
    }
  }
